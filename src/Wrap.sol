...
abstract contract Bridge is IWrap, AccessControlEnumerable {
    ...
    Multisig.DualMultisig private multisig;
    ...
    modifier isValidTokenAmount(address token, uint256 amount) {
        TokenInfo memory t = tokens[token];
    ...
    function hashRequest(uint256 id, address token, uint256 amount, address to)
        internal 
        pure 
        returns(bytes32) 
    {
    ...
    // @inheritdoc IWrap
    function reject(uint256 id, address token, uint256 amount, address to) 
        external
        isValidTokenAmount(token, amount)
    {
        bytes32 hash = hashRequest(id, token, amount, to);
    ...
    function configureToken(address token, TokenInfo calldata tokenInfo) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if(tokenInfo.maxAmount == 0 || (tokenInfo.minAmount > 0 && tokenInfo.minAmount < tokenInfo.maxAmount))
